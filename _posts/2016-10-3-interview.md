...
- 设计后端接口时，注意思考这样的几个状态：成功、失败、是否幂等、超时、异常、每个状态如何处理、是否做到了请求次数的最少设计。
  ...
  2. RESTful API 设计规范：资源命名用名词复数，HTTP方法对应CRUD，版本控制（/api/v1/）
  3. 接口安全：身份认证（Token/OAuth2）、参数校验、防重放攻击（timestamp + nonce）
  4. 接口文档：Swagger/OpenAPI 规范，入参出参示例，错误码文档
  5. 接口性能：分页查询避免全量返回、字段过滤（fields参数）、批量接口减少网络往返
...
- 分布式最终一致性方案:
  - 基于消息队列+定时补偿机制
  - 引入异步回调机制，通知机制
  - TCC（Try-Confirm-Cancel）：两阶段补偿型事务，Try预留资源，Confirm确认，Cancel回滚
  - Saga模式：长事务拆分为多个本地事务，每个事务有对应的补偿操作，支持正向恢复和反向补偿
  - 本地消息表：业务操作和消息写入同一事务，定时任务扫描未发送消息进行补偿投递
  - 最大努力通知：适用于对一致性要求不高的场景，如支付结果通知，失败后按策略重试
