...
layout: post
title: 七周七并发模型
excerpt: 从线程锁到Lambda架构：掌握七种并发编程范式
category: Distributed
tags: [并发编程, 函数式编程, CSP, MapReduce, Lambda架构]
---
...
> **结论先行**: 本书系统介绍七种并发模型，从传统线程锁到函数式编程、CSP、数据并行、Lambda架构，帮助开发者针对不同场景选择最合适的并发方案。
>
> **支撑论点**:
> 1. **线程与锁**：基础模型，需关注竞态条件、死锁、内存可见性三大危害，`ReentrantLock`和原子变量可突破内置锁限制
> 2. **函数式编程**：通过不可变状态消除并发风险，Clojure的`reduce`模式天然支持并行化
> 3. **Lambda架构**：批处理层+加速层组合，Hadoop负责批处理、Storm负责实时处理，适合TB级数据分析

### SWOT 分析

| 维度 | 分析 |
|------|------|
| **S** 优势 | 七种模型对比全面、理论结合实践、覆盖从微观到宏观的并发场景 |
| **W** 劣势 | 部分章节需要Clojure基础、OpenCL/GPU编程门槛较高 |
| **O** 机会 | 建立并发编程全局观、为微服务/大数据架构选型提供参考 |
| **T** 威胁 | 技术演进快（如Spark替代Hadoop MapReduce）、需持续更新知识 |

### 适用场景

- **推荐使用**：理解并发编程范式、选择并发模型、大数据架构设计
- **谨慎使用**：仅需Java并发的场景（可直接看线程章节）
...
#### 总结:粗略过完，在数据不变性编程思维上有更深刻的认识和理解，在不变性为前提下对函数式编程，并行，并发粗略了解，并在以此认识到在大数据分布式中是如何解决传统的问题的，后续需要重读。重读前建议先深入学习 Clojure 语言基础以及 CSP 相关论文。
