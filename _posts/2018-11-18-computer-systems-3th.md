---
layout: post
title: 计算机系统原理：从位运算到存储器层次的深度解析
excerpt: 系统讲解计算机底层原理，包括信息编码、存储器层次、位运算与逻辑运算的关键区别及Amdahl定律
category: CS
tags: [计算机系统, 位运算, 存储器层次, Amdahl定律, 大端小端]
---

## Executive Summary

### 核心观点（金字塔原理）
> **结论先行**: 理解计算机系统需要掌握"信息就是位+上下文"的核心思想，以及存储器层次结构的局部性原理。
>
> **支撑论点**:
> 1. 源程序本质是0和1组成的位序列，8位组成字节表示文本字符
> 2. 存储器层次从寄存器到远程存储形成速度-成本金字塔，利用局部性原理优化性能
> 3. 位运算与逻辑运算存在本质区别，算术右移与逻辑右移的行为差异需特别注意

### SWOT 分析

| 维度 | 分析 |
|------|------|
| **S** 优势 | 深入讲解位运算细节，包含Java语言特有的无符号数处理技巧 |
| **W** 劣势 | 乘法等章节标记TODO，机器级表示部分内容缺失 |
| **O** 机会 | 位运算和字节序知识对网络编程、跨平台开发有直接应用价值 |
| **T** 威胁 | 异或交换元素的边界条件陷阱容易被忽略，可能导致生产bug |

### 适用场景
- 深入理解底层数据表示与位操作
- 跨平台开发中的字节序问题排查
- 编写高性能代码时的缓存优化

---

##### 第1章 计算机系统漫游
###### 1.1 信息就是位+上下文
- 源程序就是一个由0和1组成的位(又称为比特)序列，8个位被组织成一组，称为字节，每个字节表示程序中的某些文本字符

```C
#include <stdio.h>
int main(){
    printf("Hello,World\n");
    return 0;
}
```

- `gcc main.c`  -> `a.out` -> `./a.out`  -> `Hello,World`
- 充分利用高速缓存，高速缓存具有`局部性`原理
- 存储器层次，左->右越来越快贵大；寄存器 -> L1高速缓存(SRAM) -> L2高速缓存(SRAM) -> L3 -> 主存(DRAM) -> 本地二级存储(本地磁盘) -> 远程二级存储(分布式文件系统,Web服务器)
- 操作系统内核是应用程序和硬件之间的媒介，它提供了三个基本抽象：1，文件时对I/O设备的抽象，2，虚拟内存时对主存和磁盘的抽象，3，进程时处理器，主存和I/O的抽象
- `Amdahl`定律：当我们对系统的某个部分加速时，其对系统整体性能的影响取决于改部分的重要性和加速程度  S=1/((1-a)+a/k))

### 第2章 信息的表示和处理
##### 2.1 字节序与位运算
- 某些机器选择在内存中按照从最低到最高有效字节的顺序存储对象，这种称之为"小端法"，反之为大端法。不同的操作系统底层的芯片时不一样的。
比如整数：0x01234567  ,小端: '.. 67 45 23 01 ...' 大端: '.. 01 23 45 67 ..'
- 异或操作两个元素交换时, `坑`注意First <= Last 是错误的，数据长度为偶数时正常，为奇数时，则First和Last都比较到中间值时异或的结果为0了，正确的是 First < Last 。
- 注意位运算与逻辑运算的区别，逻辑运算是:`ture或false` 对应的操作是 `&& ||` 注意与位运算的区别，注意在做右移位操作时，逻辑移位是左边补0，而算数逻辑是补最高位相同的值，比如: 0xC3, 1100 0011  >> 2 逻辑右移结果: 0011 0000 而算数右移结果: 1111 0000, 几乎所有的编译器对有符号数都是使用算术右移。在Java中，>> 表示算术右移，而 >>> 表示逻辑右移。
- Java没有无符号数据类型，都是采用补码形式的数据。可以进行转化，比如 一个byte x    x = x & 0x0FF 就转化为无符号数了。不过需要注意范围，可能出错，
比如 `1000 0100` 表示  -124 而转为无符号时，变成 132这就出错了。就是说`1000 0100` 即是无符号132的表示也是有符号 -124的表示。规律: 132+124-1=255


#### 程序的机器级表示
- 程序编码：`gcc -Og -S main.c` 生成汇编代码，`-c` 生成目标文件，`objdump -d` 反汇编查看机器码
- 数据格式：x86-64 中 byte(b)=1字节, word(w)=2字节, double word(l)=4字节, quad word(q)=8字节
- 寄存器：16个通用寄存器（%rax~%r15），%rsp 栈指针，%rdi/%rsi/%rdx/%rcx/%r8/%r9 用于函数参数传递（前6个参数）
- 操作数寻址：立即数（$0x1F）、寄存器（%rax）、内存引用（(%rax), 8(%rbp), (%rdi,%rsi,4)）
- 条件码与跳转：CF/ZF/SF/OF 四个条件标志位，`cmp`/`test` 设置条件码，`je`/`jne`/`jg`/`jl` 条件跳转
- 栈帧与函数调用：`call` 压入返回地址并跳转，`ret` 弹出返回地址；栈帧保存局部变量和被调用者保存的寄存器
